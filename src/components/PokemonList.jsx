...
// Define applySearch function before it's used
const applySearch = (pokemonList, search, setFilteredList) => {
...
export default function usePokemonList() {
  ...
  const hydratePokemonDetails = async (list) => {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        ...
        // Only update filtered list if search is empty (to avoid disrupting active searches)
        const currentSearch = searchRef.current;
        if (!currentSearch) {
          setFilteredList(updatedList.slice());
        } else {
          // Re-apply current search to ensure filtered list stays consistent
          applySearch(updatedList, currentSearch, setFilteredList);
        }
  ...
  useEffect(() => {
    if (pokemonList.length > 0) {
      applySearch(pokemonList, search, setFilteredList);
    }
  }, [search, pokemonList]);
